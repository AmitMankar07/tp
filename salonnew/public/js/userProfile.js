...
console.log("toejb",token);
const decodedToken = decodeToken(token);
console.log("toejb",decodedToken);
const userId = decodedToken.id;
console.log("userid:",userId);

getDonationHistoryBtn.addEventListener('click', async () => {
    try {
      const response = await axios.get('/api/auth/donation-history',{ headers: { Authorization: token }} );
...
async function getProfile() {
    try {
        const response = await axios.get('/api/auth/user/get-profile',{ headers: { Authorization: token }, params: { userId: userId }} );
        console.log(response);
        const userProfile = response.data.user;
        profileNameElement.textContent = `Name: ${userProfile.username}`;
...
updateProfileForm.addEventListener('submit', async (event) => {
    ...
    try {
        const response = await axios.put('/api/auth/user/update-profile', {
            name,
            email,
        },
        { headers: { Authorization: token },params: { userId: userId }},
    );
        console.log("updt resopone",response);

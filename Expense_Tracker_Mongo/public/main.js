...
console.log("downlaod responsne:",response);
    ...
    const token = localStorage.getItem('token');
        ...
        var options={
            ...
            "handler":async function (response){
                console.log("in the update api");
                
                const userId = parseJwt(token).userId;
                console.log(userId); 
              const updtresponse=  await axios.post('http://localhost:3000/users/premium/updateTransactionStatus',{
                    order_id:options.order_id,
                    payment_id:response.razorpay_payment_id,
                    userid: userId  
                },{headers:{'Authorization':token}})

                console.log("the update api success")
                 console.log("update response",updtresponse);
...
async function fetchAndDisplayExpenses(page = 1) {
    ...
    async function deleteExpense(expense) {
        ...
        console.log("expenseid in delete",expenseId);
        try {

...
const getAllExpenses=async (req,res,next)=>{
    try{
      ...
      //     }]
      // });
      const count = await expenses.countDocuments({ userId });
      console.log("count",count);
      // Get the expenses for the user with pagination
      const userExpenses = await expenses.find({ userId })
          .skip(parseInt(offset))
          .limit(parseInt(limit))
          .populate('userId', 'id name'); // Assuming 'user' is the reference field in your Expense model
...
const deleteUserExpenses = async (req, res, next) => {
    ...
    try {
      console.log("Request Params:", req.params); 
        const { id } = req.params; // Extract expense ID from request parameters
        console.log("Expense ID:", id); // Log the ID to check its value
        ...
        const deletedExpense = await expenses.findById(id); // Find expense by ID
        if (!deletedExpense) {
            return res.status(404).json({ message: 'Expense not found' });
        }
        // Check if the user ID in the token matches the user ID of the expense
        if (deletedExpense.userId.toString() !== userId) {

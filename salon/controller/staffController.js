...
console.log(StaffService.associations);
...
exports.assignServices = async (req, res) => {
    try {
        ...
        console.log(`StaffCOntroller ${staffId} and serviveCotroller ${serviceIds}`)
        ...
        console.log(`Staff Databse ${staff}`)
        if (!staff) {
        ...
        console.log(`Staff Databse ${services}`)
    ...
    try {
        const assignments = await StaffService.findAll({
            include: [
                {
                    model: Staff,
                    // If you have defined an alias in the association, use it here
                    // as: 'Staff', // 
                    },
                {
                    model: Service,
        ...
        console.error('Error loading getAssignments:', error);
        res.status(500).json({ error: 'An error occurred while loading assignments.' });
    }
};
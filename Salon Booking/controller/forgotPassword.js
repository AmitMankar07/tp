...
const SendEmail=async(req,res,link,email)=>{
    const sender={
        email:'amitmankar1920@gmail.com'
    }
...
console.log("user in forgot pss:",user);
        if (!user) {
        ...
        if (user) {
            const link = `http://localhost:4000/password/resestpassword/${uuid}`;
...
exports.sendForm = async (req, res, next) => {
    ...
    const htmlContent = `
        <form action="http://localhost:4000/password/resestpassword/${uuid}" method="POST" id="reset-passsword-form" class="form"> 
        ...
        <script>
            document.getElementById("reset-passsword-form").addEventListener("submit", async (e) => {
                ...
                try {
                    const response = await axios.post('http://localhost:4000/password/resestpassword/${uuid}', { pass: password });
